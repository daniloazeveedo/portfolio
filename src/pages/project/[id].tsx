...
import { ArrowLeft, ChatCenteredText, Image as IconImage, YoutubeLogo, Hash } from 'phosphor-react'
...
export default function Projeto({ project }: ProjectProps) {
  return (
    <>
      <Head>
        <title>{project.title} | Danilo Azevedo </title>
      ...
      <S.ProjectContainer>
        <S.Banner>
          <img className="bannerUrl" src={project.banner} alt={`Banner do projeto ${project.title}`} />
        ...
        <S.PrintContainer>
          ...
          <S.PrintContent>
            ...
            <S.Print>
              {project.print && project.print.length > 0 && project.print.map(print => (
        ...
        <S.ContainerVideo>
          <div className="imageVideo">
            <img src={project.backgroundImage} alt={`Imagem de fundo do projeto ${project.title}`} />
          </div>
